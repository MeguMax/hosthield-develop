...
export default function MyWaiver() {
  ...
  useEffect(() => {
    ...
    // get properties
    const url = `${PUBLIC_API_KEY}/waiver-bookings?username=${userItem.value.username}&waiver=${router.query.waiverId}`;
    fetch(url, {
      method: 'GET',
      credentials: 'include',
      headers: {
      ...
      {isLoading ?
        ...
        <div className="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:justify-between">
          <h3 className="text-3xl lg:text-4xl font-semibold">My Waivers</h3>
              <Link href="/dashboard/user/create-booking">
          ...
          {properties.map((item) => (
            ...
            >
              ...
              <div className="flex justify-center items-center space-x-1 pt-2 px-2">
                ...
                {item.signedAmount} / {item.requiredAmount}
                </span>
              </div>
              <div className="flex relative justify-evenly items-center border-t-2 divide-x divide-site-darkgray divide-opacity-40 border-gray-100 mt-4">
                <div className="w-2/4 flex py-2 justify-center items-center relative">
                  <ToolTip
                    ...
                    classNames="-top-9"
                  >
                    <Link href={`/dashboard/user/view-waiver?waiverId=${item.waiverId}&propertyName=${item.waiverName}&status=${item.status.status}&bookingId=${item.id}`}>
                ...
                <div className="w-2/4  flex py-2 justify-center items-center relative ">
                  ...
                  >
                    {properties.length==1 ?
                      (<RiShareForwardFill
                          data-for="tooltip-share"
                          data-tip="Share your waiver using this button"
                          className="text-xl text-site-dark text-opacity-50 hover:text-site-main focus:text-site-main"
                          onClick={() => {
                            if (item.status.status === 'approved') {
                              dispatch(
                                setShareModalLink(
                                  `https://app.shieldmyrental.com/guest-sign-waiver?id=${item.waiverId}&bookingId=${item.id}`
                                )
                              );
                        ...
                        onClick={() => {
                          if (item.status.status === 'approved') {
                            dispatch(
                              setShareModalLink(
                                `https://app.shieldmyrental.com/guest-sign-waiver?id=${item.id}&bookingId=${item.id}`
                              )
                            );
                            dispatch(setShareModalIsOpen(!shareModal.isOpen));
                          }
